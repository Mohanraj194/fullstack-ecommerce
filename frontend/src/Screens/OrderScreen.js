...
const OrderScreen = ({history,match}) => {
    ...
    }, [dispatch,orderId,order,successPay,history,successDeliver,userInfo])
    ...
    if(!loading && !error)
    {
        ...
        if(!order.itemsPrice)
        {
            ...
            order.itemsPrice = decimal(order.orderItems.reduce((acc,item)=>acc+item.qty*item.price,0))
        }
                ...
                <ListGroup variant='flush'>
                    <ListGroup.Item>
                        ...
                        <p>
                            ...
                            {order.shippingAddress.address}, {order.shippingAddress.city}, {order.shippingAddress.portalCode}, {order.shippingAddress.country}</p>

                            {order.isDelivered ? (<Message variant="success">Paid on {order.delivaredAt}</Message>):
